...
enum Mode {
    ...
    Ps {
        ...
        /// Path to a PowerShell script to execute
        #[arg(long, group = "ps_input")]
...
fn main() -> Result<(), String> {
    ...
    match cli.mode {
        Mode::Clr {
            ...
            // Use the args from the Mode::Clr variant
            let exe_args = args;
            ...
            let (data, key_bytes, iv_bytes) = if let (Some(key_path), Some(iv_path), Some(assembly_path)) = (key, iv, assembly) {
                ...
                #[cfg(not(feature = "compiled_clr"))]
                {
                    // Handle the absence of compiled data
                    println!("Compiled data is not included in this build. Enable the 'compiled_clr' feature to include it.");
                    std::process::exit(0);
                }
            };
            ...
            match setup_bypass() {
                Ok(_) => {
                    ...
                    let mut clr = Clr::new(decrypted_data, exe_args)
                ...
                Err(err_msg) => {
                    println!("Error during bypass setup: {}", err_msg);
                }
            }
        }
        ...
        Mode::Bof { args, base, key, iv, bof } => {
            ...
            let (bof_data, key_bytes, iv_bytes) = if let (Some(key_path), Some(iv_path), Some(bof_path)) = (key, iv, bof) {
                ...
                {
                    // Handle the absence of compiled data
                    println!("Compiled BOF data is not included in this build. Enable the 'compiled_bof' feature to include it.");
                    std::process::exit(0);
                }
            };
            ...
            match setup_bypass() {
                Ok(_) => {
                    ...
                    match Coffee::new(&decrypted_bof_data) {
                        Ok(coffee) => {
                            ...
                            match coffee.execute(
                                ...
                                Err(e) => println!("[!] BOF execution failed: {}", e),
                            }
                        },
                        Err(e) => return Err(format!("[!] Failed to load BOF: {:?}", e)),
                    }
                },
                Err(err_msg) => return Err(format!("Error during bypass setup: {}", err_msg)),
            }
        }
    }
